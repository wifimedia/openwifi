...
model_hardware=$(cat /proc/cpuinfo | grep 'machine' | cut -f2 -d ":" | awk '{print $1}')
...
uri=`uci -q get wifimedia.@heartbeat[0].uri`
heartbeat_uri=`uci -q get wifimedia.@heartbeat[0].heartbeat_uri`
